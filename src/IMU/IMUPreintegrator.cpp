...
//        Ca*IMUData::getAccMeasCov()*Ca.transpose();
	...
	std::ostream & operator <<(std::ostream &out, IMUPreintegrator &preint){
		out <<preint._delta_time<<" "<<preint._delta_P.transpose() << " " << preint._delta_V.transpose() << " " << preint._delta_R.transpose();
		return out;
	}
